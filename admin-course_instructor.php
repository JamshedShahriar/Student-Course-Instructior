...
<head>
  <meta charset="UTF-8">
  <title>Course Instructor Allocations</title>
...
<div class="header-panel">
  <div class="container d-flex justify-content-between align-items-center">
    <h2 class="m-0 font-weight-bold">🌿 Faculty Allocations</h2>
    <div>
...
<div class="container">
  <div class="table-container">
    <h5 class="mb-3 font-weight-bold text-dark">Active System Distribution Map</h5>
    <div class="table-responsive">
      <table class="table table-hover align-middle">
        <thead>
          <tr>
            <th>Course ID Code</th>
            <th>Instructor ID Code</th>
        ...
        <tbody>
          ...
          while ($row = $result->fetch_assoc()) {
              echo "<tr>
                      <td>Course ID: <strong>#{$row['Course_ID']}</strong></td>
                      <td>Instructor ID: <strong>#{$row['Instructor_ID']}</strong></td>
                      <td>{$row['Assignment_Date']}</td>
                      <td class='text-center'>
                        <a href='edit_course_instructor.php?course_id={$row['Course_ID']}&instructor_id={$row['Instructor_ID']}' class='btn btn-sm btn-outline-primary me-1'>Edit</a>
                        <a href='delete_course_instructor.php?course_id={$row['Course_ID']}&instructor_id={$row['Instructor_ID']}' class='btn btn-sm btn-outline-danger' onclick=\"return confirm('Are you sure?');\">Delete</a>
                      </td>
                    </tr>";
          }
          ?>
